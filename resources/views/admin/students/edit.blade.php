            ...
            @method('PUT')
                ...
                <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
                    ...
                    <div class="card card-flush py-4">
                        ...
                        <input type="text" value="{{$row->email ?? ''}}" name="email" class="form-control @error('email') is-invalid @enderror">
                        @error('email')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    ...
                    <div class="mb-3">
                        <label class="form-label">geloos_no</label>
                        <input type="text" value="{{$row->geloos_no}}" name="geloos_no" class="form-control @error('geloos_no') is-invalid @enderror">
                        @error('geloos_no')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    @enderror
                        ...
                        <div class="mb-3">
                            <label class="form-label">nationality_id</label>
                            <input type="text" value="{{$row->nationality_id}}" name="nationality_id" class="form-control @error('nationality_id') is-invalid @enderror">
                        ...
                        <div class="mb-3">
                            ...
                            <select required class="form-select form-select-solid dynamic"
                                ...
                                data-show-subtext="true" name="level_id" data-live-search="true" id="country"
                                data-dependent="sub">
                                <option value=""></option>
                    ...
                    <div class="mb-3">
                        ...
                        <select required class="form-select form-select-solid dynamic"
                            data-control="select2" data-placeholder="Select an option" required
                            data-show-subtext="true" name="department_id" data-live-search="true" id="country"
                            data-dependent="sub">
                            <option value=""></option>
                            @foreach ($departments as $department)
                                <option value="{{ $department->id }}" {{$row->department_id == $department->id ? "selected" :""}} >{{ $department->name ?? '' }}
                                </option>
                            @endforeach
                        </select>
                </div>
